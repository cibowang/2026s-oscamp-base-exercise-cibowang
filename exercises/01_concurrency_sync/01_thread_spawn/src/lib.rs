...
pub fn double_in_thread(numbers: Vec<i32>) -> Vec<i32> {
    ...
    let th = std::thread::spawn(move || numbers.iter().map(|e| e * 2).collect());
    th.join().unwrap()
}
...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    ...
    let th1_handle = std::thread::spawn(move || a.iter().copied().sum::<i32>());
    let th2_handle = std::thread::spawn(move || b.iter().copied().sum::<i32>());
    let res_1 = th1_handle.join().unwrap();
    let res_2 = th2_handle.join().unwrap();
    (res_1, res_2)
}
...
pub fn increment_thread_local() -> usize {
    ...
    THREAD_COUNT.with(|id| *id.borrow_mut() += 1);
    THREAD_COUNT.with(|id| *id.borrow())
}
...
pub fn scoped_slice_sum(a: &[i32], b: &[i32]) -> (i32, i32) {
    ...
    let (sum_a, sum_b) = std::thread::scope(|s| {
...
pub fn handle_panic(value: i32, should_panic: bool) -> Result<i32, ()> {
    ...
    let handle = std::thread::spawn(move || match should_panic {
        true => Err(()),
        false => Ok(value),
    });
    handle.join().unwrap()
