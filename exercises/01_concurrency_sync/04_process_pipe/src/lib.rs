...
pub fn run_command(program: &str, args: &[&str]) -> String {
    ...
    let parent_process = Command::new(program)
        .args(args)
        .stdout(Stdio::piped())
        .output()
        .unwrap();
    String::from_utf8(parent_process.stdout).unwrap()
}
...
pub fn pipe_through_cat(input: &str) -> String {
    ...
    let mut child_process = Command::new("cat")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
        .unwrap();
    ...
    drop(stdin);
    // No need to define stdout
    // No need to read_to_string()
    let output = child_process.wait_with_output().unwrap();
    String::from_utf8(output.stdout).expect("vvvv")
}
...
pub fn run_command_with_result(program: &str, args: &[&str]) -> io::Result<String> {
    ...
    let process = Command::new(program)
        ...
        .stdout(Stdio::piped())
        .output()?;
    Ok(String::from_utf8(process.stdout).map_err(|_| ErrorKind::InvalidData)?)
}
...
pub fn pipe_through_grep(pattern: &str, input: &str) -> String {
    ...
    drop(stdin);
    let output = child_process.wait_with_output().unwrap();
    String::from_utf8(output.stdout).expect("must be string")
}
